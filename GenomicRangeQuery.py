...
'''
''' Thoughts:
Find the prefix sums, or the number of total occurrence, of each letter. Within indexes of P[i] and Q[i] of prefix sums
 of each letter, if the value of the difference between two ends are different, then this letter has occurred. 
'''
...
def solution(S, P, Q):
    ...
    increment_G = 0
    increment_T = 0
    pastOccurrence_A = [0] * (n + 1)
    pastOccurrence_C = [0] * (n + 1)
    pastOccurrence_G = [0] * (n + 1)
    pastOccurrence_T = [0] * (n + 1)
    result = []
    for i in range(n):
        if S[i] == 'A':
            increment_A += 1
            ...
            increment_T += 1
        pastOccurrence_A[i+1] += increment_A
        pastOccurrence_C[i+1] += increment_C
        pastOccurrence_G[i+1] += increment_G
        pastOccurrence_T[i+1] += increment_T
    print("pastOccurrence_A = ", end = "")
    print(pastOccurrence_A)
    print("pastOccurrence_C = ", end="")
    print(pastOccurrence_C)
    print("pastOccurrence_G = ", end="")
    print(pastOccurrence_G)
    print("pastOccurrence_T = ", end="")
    print(pastOccurrence_T)
    for j in range(m):
        if pastOccurrence_A[Q[j] + 1] - pastOccurrence_A[P[j]] > 0:
            result += [1]
        elif pastOccurrence_C[Q[j] + 1] - pastOccurrence_C[P[j]] > 0:
            result += [2]
        elif pastOccurrence_G[Q[j] + 1] - pastOccurrence_G[P[j]] > 0:
            result += [3]
        elif pastOccurrence_T[Q[j] + 1] - pastOccurrence_T[P[j]] > 0:
            result += [4]
    return result

print(solution('GTAACCGAAC', [3, 0, 0, 4, 4, 8, 9], [4, 0, 1, 6, 7, 9, 9]))